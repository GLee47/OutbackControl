...
//#define WATERHEATEROFF	if (INVERTER_AUX_OUT!=0){cmdMate("AUXOFF","1");}
...
char /*buf[1024],*/ strtmp[256];
//FILE *fp;
WINDOW * InvWin,* CCWin,* FNDCWin,* ScrollWin;
int ch;
bool initilized=NO, EnableSystem=NO, terminate=false, DischargeAllowed=YES, 
				preferHeaterOn=FALSE,InvChrgEnabled=NO, MateCommResponseExpected=NO;
...
float BatAmpHrIn=0.0, BatAmpHrOut=0.0, BatAmpHrCrx=0.0, MaxVNegAmps=0.0,BattEffFac=0.91;
float SellWH,BuyWH,InvWatts,InvWH, netbattamps=0.0, BatTrgAmp=0.0, MaxNegBatAmpsDropped=(-MAX_NEG_NBA_DROPPED);
float WHtopMaxTemp=150.0,WHtopMinTemp=100.0,WHCenterMinTemp=50.0;
...
PrgDataStruct;
/*
int BufInt(int pos,int len){
	buf[pos+len]=0;
	return(atoi(buf+pos));
}
	
long BufLong(int pos,int len){
	buf[pos+len]=0;
	return(atol(buf+pos));
}
*/
...
//	BatAmpHrCrx=(100.0-(float)(FNDC_SOC))*(BatRatedAmpHr/100.0);
	...
	if((hour>=12) && (hour<=15))WHtopMinTemp=120;
	if((hour>15) && (hour<=17)){
		WHCenterMinTemp=110;
		WHtopMinTemp=130;
	}
...
//6:15PM
	if (TimeTest(18,15,0) /*&& InvAuxLockSaved!=Undefined*/){
		whsSetFlags(whsTimer, whsOff);
...
/*		{
			if(msgBuf[0]=='P')
				ProgramIndx=atoi(msgBuf+1);
		}*/
//		if( strcasecmp(SMS_Rec_Mesg(MsgBuf),"
	}
}
...
/*	{static unsigned long reSendTimer=0ul;
	...
	if (FNDC_BATT_VOLTS<54){
		...
		if (FNDC_BATT_VOLTS>56) whsSetFlags(whsConserve,whsOff);
		if (preferHeaterOn) digitalWrite(27,1);
	}
}
...
//					cmdMate("SELLV","-",__LINE__);
	...
	{
		if (((weight<0) && (digitalRead(26)==1) && sellv>= SellVoltMax ) ||
			((weight>0) && ((digitalRead(26)==0) && sellv<= SellVoltMin ) ))
		{
			weight=0;
		}	
	}
...
int SOC=(int)((((float)BATT_STATUS_AH+ (float)BatRatedAmpHr)/(float)BatRatedAmpHr)*100.0); //FNDC_SOC;
	...
	if((FNDC_BATT_VOLTS>52) && (preferHeaterOn)) digitalWrite(27,1);
	if(FNDC_BATT_VOLTS<51.2) digitalWrite(27,0);
	if((FNDC_BATT_VOLTS<56)  && (!preferHeaterOn))digitalWrite(27,0);
...
int CC1Mode=CC1_MODE, CC2Mode=CC2_MODE;
	...
	if (InvChrgEnabled==YES){
		if ((CC1_PVIV>50) || (CC2_PVIV>50)){
			cmdMate("CHG","0",__LINE__);
			InvChrgEnabled=NO;
		}
	}
	UnderUtilization=((CC1Mode==1) || (CC1Mode==3) || (CC2Mode==1) || (CC2Mode==3));
	ngp = ((L1_BUY_AMPS-L1_SELL_AMPS)*L1_INVERTER_VOUT)
	...
	if (netbattamps	> 0.0){
		...
		if ((ContinousNegAmpsSec>0) || (netbattamps<(-1.0))){ 
			ContinousNegAmpsSec++;
			if ((FNDC_BATT_VOLTS>MaxVNegAmps)&&(ContinousNegAmpsSec>60)) MaxVNegAmps=MIN(FNDC_BATT_VOLTS,51.6);
		}
	}	
	...
	if (((L1_INVERTER_AMPS+L1_BUY_AMPS-L1_SELL_AMPS)>45) || ((L2_INVERTER_AMPS+L2_BUY_AMPS-L2_SELL_AMPS)>45)){
		if (digitalRead(26)!=0) cmdMate("AUXOFF","1",__LINE__);
		else 
		{
...
/*			static unsigned long reSendTimer=0ul, veriCntStTick=0ul, retriggerDelay=300ul;
			static int veriCnt=0;
			const int reqTriggers=3, reqTriggersWithin=4;
			if ((veriCnt==0) && (veriCntStTick==0ul))
			{
				veriCnt=1;
				veriCntStTick=Ticks;
				wprintw(ScrollWin,"%d Debug: set veriCnt=%d veriCntStTick=%ul Ticks=%ul\n",__LINE__,veriCnt,veriCntStTick,Ticks);
			}
			if ((veriCntStTick+reqTriggersWithin)<Ticks)
			{
				wprintw(ScrollWin,"%d Debug: veriCnt=%d veriCntStTick=%ul Ticks=%ul\n",__LINE__,veriCnt,veriCntStTick,Ticks);
				veriCnt=1;
				veriCntStTick=Ticks;
			}
			else
			{
				if (veriCnt>=reqTriggers)
				{
					if (reSendTimer < Ticks)
					{
						int L1amps=(L1_INVERTER_AMPS+L1_BUY_AMPS-L1_SELL_AMPS);
...
*/		}
	...
	if (pause>0){
		...
		switch (whGetDesiredState(digitalRead(26),INVERTER_AUX_OUT,
...
int SellWatts,BuyWatts,InvWatts;	
	...
	const int WH_Amps_120v=10,WH_Amps_240v=20;	
		...
		{
			if ((L1_INVERTER_AMPS+L1_BUY_AMPS-L1_SELL_AMPS)>=WH_Amps_120v) IAO_Day_Secs++;	
		}
	}
}
...
void printStuff(void){
	...
	WMVPRINTW(InvWin,8,2,"Aux   %2d %2d",(digitalRead(26)!=0), (INVERTER_AUX_OUT!=0));
...
//	ResetTime_p=localtime(&ResetTime);
	WMVPRINTW(CCWin,10,2,"%02d:%02d:%02d  %02d:%02d:%02d",hour,minute,second,
										ResetTime_p.tm_hour,ResetTime_p.tm_min,ResetTime_p,ResetTime_p.tm_sec);
	WMVPRINTW(CCWin,11,2,"M%1d A%1d T%1d DT%1d C%1d S%1d",whsSetFlags(whsManual,whsInquiry),whsSetFlags(whsAuto,whsInquiry),
			whsSetFlags(whsTimer,whsInquiry),whsSetFlags(whsDisableTimer,whsInquiry),
			whsSetFlags(whsConserve,whsInquiry), whGetDesiredState(digitalRead(26),INVERTER_AUX_OUT,
			(L1_INVERTER_AMPS+L1_CHARGER_AMPS+L1_BUY_AMPS-L1_SELL_AMPS),
			(L2_INVERTER_AMPS+L2_CHARGER_AMPS+L2_BUY_AMPS-L2_SELL_AMPS)));
	WMVPRINTW(CCWin,12,1,"T %5.1f C %5.1f   ",  WHtopMinTemp, WHCenterMinTemp);
	WMVPRINTW(CCWin,13,1,"%5.1f %5.1f %5.1f %5.1f",sensor[0].tempF,sensor[1].tempF,sensor[2].tempF,sensor[3].tempF);
