...
char *
quotedString (char *s, int MaxChars)
{
//      printf("%s\n",s);
  int i, first = 0;
...
char *
trim (char *s)			//returns a pointer to string without leading or trailing spaces
{
...
int
isInStr (char *str, char *find)	//returns 1 if find is in str, 0 if not
{
  if (str[0] == find[0])
    {
      if (find[1] == 0)
...
int
SMS_SendMsg (char *PhNum, char *Msg)
{
  ...
  if (!PortOpened)
    if (!openPortRaw ())
      return 0;
  if (chat ("AT\r", "OK", 3, NULL))
    return 0;
  if (chat ("AT+CMGF=1\r", "OK", 3, reply))
    return 0;
  sprintf (tBuf, "AT+CMGW=\"%s\"\r", PhNum);
  if (chat (tBuf, ">", 3, reply))
    return 0;
  sprintf (tBuf, "%s\032", Msg);
  if (chat (tBuf, "+CMGW:", 3, reply))
    return 0;
...
char *
SMS_Rec_Mesg ( /*char */ struct SMS_Message_Properties *buff)
{
  char reply[0x1000], mn[0x10], *msgNum, tBuf[0x100], *mType, *pNum, *dtime,
    *message;
  int i;
  if (!PortOpened)
    if (!openPortRaw ())
  ...
  if (chat ("AT+CMGF=1\r", "OK", 3, reply))
    return NULL;
  if (chat ("AT+CMGL=\"REC UNREAD\"\r", "+CMGL:", 3, reply))
    return NULL;
//      printf("%s\n",reply);
  for (i = 0; i < strlen (reply); i++)
    {
...
//      printf("-%s-\n",pNum);
  ...
  if ((pNum == NULL) || (dtime == NULL) || (message == NULL))
    return NULL;
  chop (message, 13);
  chop (message, 10);
