...
syncACPS (void)
{
	AirCondPwrSrc =
		(digitalRead (MRCOOL2KHP_SRC_GPIO) ==
		 0) ? acpsGrid : ((digitalRead (MRCOOL2KHP_PWR_GPIO) == 1) ? acpsInverter : acpsNone);
}
...
airCondControl (int targetState)
{
	...
	if ((difftime (time (NULL), tChangeTime) > MIN_SECS_BETWEEN_CHANGES) || (targetState > ON))
	  {
	...
	if (difftime (time (NULL), AirJordanSwitchTime) > 300)
	  {
		  if (((desired == 1) && ((TOT_LOAD (L1) + AIR_COND_AMPS) < MAX_LOAD_AMPS)
		       && ((TOT_LOAD (L2) + AIR_COND_AMPS) < MAX_LOAD_AMPS) && ((TEMPSENSOR (TOP) > 120.0)
										|| ((UE_PWM.percent > 80) && LE_IS_ON)
										|| (vacation == TRUE)
...
{
	int overTemp =
		(MAX (MAX (TEMPSENSOR (TOP), TEMPSENSOR (BOTTOM)), MAX (TEMPSENSOR (CENTER_RIGHT), TEMPSENSOR (CENTER_LEFT))) >
		 MAX (WHMAXTEMP, MAX (WHtopMinTemp + 1, WHCenterMinTemp + 1)));
	float midTankTemp =
		(((TEMPSENSOR (CENTER_LEFT) * 55.0) + (TEMPSENSOR (BOTTOM) * 20.0) + (TEMPSENSOR (CENTER_RIGHT) * 7.5) +
		  (TEMPSENSOR (TOP) * 17.5)) / 100.0);
	...
	if ((!LE_IS_ON_FULL) && (midTank == irBelow) && (INVERTER_AC_MODE != iacmNoGr)
	    && ((TOT_LOAD (L2) + LOWER_LV_L2_DIFF) < MAX_LOAD_AMPS))
	  {
		  //lower tank below min temp -- turn on lower element, set delay, and return
		  AlterCurrentLimit ((TOT_LOAD (L2) + LOWER_LV_L2_DIFF), (0 - __LINE__), 1);
	...
	if ((UE_PWM.percent < 100) && (topTank == irBelow) && (INVERTER_AC_MODE != iacmNoGr)
	...
	if ((INVERTER_AC_MODE == iacmNoGr))	//grid down 
	  {
		  ...
		  if ((AirCondPwrSrc == acpsInverter) && (AFNDCV < (fSellV - 2.2)))
		  ...
		  else if (AFNDCV > (fSellV + 1.0))
		    {
			    ...
			    if ((AirCondPwrSrc == acpsGrid) && ((TOT_LOAD (L1) + 10) < MAX_LOAD_AMPS)
	...
	else if ((INVERTER_AC_MODE == iacmUse) && (!GT_NoDrop))	//using grid -- minimize use unless grid tied and no drop selected
	  {
		  ...
		  if (topTank != irBelow)
		    ...
		    {
			    //turn on top element
			    if ((UE_PWM.percent < 100) && ((EstL1A + UPPER_L1_DIFF) <= MAX_LOAD_AMPS) && (INVERTER_AUX_OUT != 0))
			      {
				      logMesg ("LC @ %d\n", __LINE__);
				      UEON;
			      }
		  ...
		  if (midTank != irBelow)
		  ...
		  else
		    {
			    ...
			    if ((!LE_IS_ON_FULL) && ((EstL2A + LOWER_LV_L2_DIFF) <= MAX_LOAD_AMPS))
			      {
				      logMesg ("LC @ %d\n", __LINE__);
				      LEON;
			      }
	...
	else if ((INVERTER_AC_MODE == iacmUse) && (GT_NoDrop))
	  {
		  ...
		  if ((!LE_IS_ON_FULL) && (midTank != irAbove) && ((TOT_LOAD (L2) + LOWER_LV_L2_DIFF) < MAX_LOAD_AMPS)  && (loadBalance== -1))
		    {
			    ...
			    if (!overTemp)
			      {
				      ...
				      loadBalance = 0;
				      lcDelay = DEFAULT_DELAY;
				      logMesg ("LC @ %d LE==%d %4.2f %4.2f %4.1fF\n", __LINE__, LE_PWM.percent, AFNDCV,
					       MAX (50.8 + MAX (0, ((TEMPSENSOR (CENTER_LEFT) - LE_SETPOINT_START) / LE_DIVISOR)),
						    MIN (MAX (fSellV, 57), fSellV + 0.8)),TEMPSENSOR(CENTER_LEFT));
				      PRETURN;
			      }
		    }
		  if ((UE_PWM.percent < 100) && (topTank != irAbove) && (!overTemp) &&
		      ((TOT_LOAD (L1) + UPPER_L1_DIFF) < MAX_LOAD_AMPS) && (loadBalance== -1))
		    {
			    //upper tank temp in range -- turn on upper element, set delay, and return
			    UE_UP;
			    lcDelay = DEFAULT_DELAY;
			    logMesg ("LC @ %d UE at %d, %4.2f %4.2f\n", __LINE__, UE_PWM.percent, AFNDCV,
...
//Dropped Grid Mode
	...
	if ((loadBalance == -1) || (UnderUtilization) || ((vacation == FALSE) && (ANBA > DCamps (LOWER_LV_L2_DIFF)))
	    || ((vacation == FALSE) && (AFNDCV > MAX (54.0, fSellV))) || ((vacation == TRUE) && (AFNDCV > VACATION_VSET)))
	  {			//excess power. Find something to turn on
		  ...
		  if ((TEMPSENSOR (TOP) < WH_TOP_NORMAL_TEMP) &&
		      ...
		      (((TOT_LOAD (L1) + (UPPER_L1_DIFF / MAX(1,UE_PWM.resolution))) < MAX_LOAD_AMPS) || (AFNDCV > 56.0))
		      && ((AFNDCV > MAX (50.7, fSellV + 0.6)) || (((100-FNDC_SOC)*2)<ANBA)) && (!GT_NoDrop))
		    {
			    //upper tank temp in range -- turn on upper element, set delay, and return
			    UE_PWM.percent += (100 / UE_PWM.resolution);
			    loadBalance = 0;
			    lcDelay = DEFAULT_DELAY;
			    logMesg ("LC @ %d UE at %d, %4.2f %4.2f\n", __LINE__, UE_PWM.percent, AFNDCV, MAX (50.7, fSellV + 0.6));
			    PRETURN;
		    }
		  //top >120 or we got more juce them bring up bottom temp
		  if ((!LE_IS_ON_FULL) && (midTank != irAbove) && ((TOT_LOAD (L2) + LOWER_LV_L2_DIFF) < MAX_LOAD_AMPS)
		  ...
		  if ((UE_PWM.percent < 100) && (topTank != irAbove) && (!overTemp) &&
		      ((TOT_LOAD (L1) + UPPER_L1_DIFF) < MAX_LOAD_AMPS) && ((AFNDCV > 56.0)|| (((100-FNDC_SOC)*2)<ANBA)) &&
		      (AFNDCV > MAX (51.2 + MAX (0, ((TEMPSENSOR (TOP) - UE_SETPOINT_START) / UE_DIVISOR)), fSellV + 1.2))  && (!GT_NoDrop))
		    {
			    //upper tank temp in range -- turn on upper element, set delay, and return
			    UE_PWM.percent += (100 / MAX(1,UE_PWM.resolution));
			    lcDelay = DEFAULT_DELAY;
			    logMesg ("LC @ %d UE at %d, %4.2f %4.2f\n", __LINE__, UE_PWM.percent, AFNDCV,
				     MAX (51.2 + MAX (0, ((TEMPSENSOR (TOP) - UE_SETPOINT_START) / MAX(1,UE_DIVISOR))), fSellV + 1.2));
			    loadBalance = 0;
			    PRETURN;
		    }
...
//      else because it returns if any above is true
	if ((((!UnderUtilization) && (AFNDCV < MAX (51.2, MIN (57, (fSellV + 2.0)))) && (preferHeatPumpOn == FALSE))
	...
	if (((!UnderUtilization) /*&& (ANBA < 0) */  &&
	     (((AFNDCV < VACATION_VSET) && (vacation == TRUE)) ||
	      (AFNDCV < MAX (51.2 + MAX (0, ((TEMPSENSOR (TOP) - UE_SETPOINT_START) / UE_DIVISOR)), MIN (57, fSellV + 1.2))))
	     && (!GT_NoDrop)) || (loadBalance == 1))
	  {
		  if ((UE_PWM.percent > 0) && (topTank != irBelow))
		    {
			    ...
			    logMesg ("LC @ %d UE_PWM.percent=%d %4.2f %4.2f\n", __LINE__, UE_PWM.percent, AFNDCV,
				     MAX (51.2 + MAX (0, ((TEMPSENSOR (TOP) - UE_SETPOINT_START) / UE_DIVISOR)),
					  MIN (57, fSellV + 1.2)));
			    loadBalance = 0;
			    PRETURN;
		    }
	...
	if (((!UnderUtilization) /*&& (ANBA < 0) */  &&
	     (((AFNDCV < (VACATION_VSET - 0.4)) && (vacation == TRUE)) ||
	      (AFNDCV <
	       MAX (50.8 + MAX (0, ((TEMPSENSOR (CENTER_LEFT) - LE_SETPOINT_START) / LE_DIVISOR)),
		    MIN (MAX (fSellV, 57), fSellV + 0.8)))) && (!GT_NoDrop)) || (loadBalance == 1))
	  {
	...
	if (((!UnderUtilization) && (AFNDCV < fSellV) && (preferHeatPumpOn == TRUE) && (!GT_NoDrop)) || (loadBalance == 1))
	  {
		  if ((digitalRead (AIR_COND_GPIO_PIN)))
		    {
			    if (airCondControl (OFF) == TRUE)
	...
	if ((vacation == TRUE) && (AFNDCV < (VACATION_VSET - 4.0)))
		setACPS (acpsNone);
	if (((!UnderUtilization)
	     && ((AFNDCV < MAX ((fSellV - 2.0), 46.0)) || ((vacation == TRUE) && (AFNDCV < (VACATION_VSET - 3.0)))) 
	     && (!GT_NoDrop)
	     && (INVERTER_AC_MODE == iacmDrop || INVERTER_AC_MODE == iacmNoGr)) 
			|| ((loadBalance == 1) && (airJordanManOff == TRUE)))
	  {
		  if (AirJorPrefered (0) != 0)
		    {
			    lcDelay = DEFAULT_DELAY;
			    logMesg ("LC @ %d  V=%6.3f  fSellv=%6.3f  loadBal=%d\n", __LINE__, AFNDCV, fSellV, loadBalance);
			    loadBalance = 0;
			    PRETURN;
		    }
	  }
