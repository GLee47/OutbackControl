...
float readSensorF(int SensorIndx, float ErrorValue){
	if((Ticks - sensor[SensorIndx].lastUpdate) > STALE_TICKS) return ErrorValue;
	return sensor[SensorIndx].tempF;
}

int getAddressIndx(long address1,long address2){
	int i=0;
	for (i=0;i<20;i++){
...
//          memset(data,0,sizeof data);
		{ int i; for(i=0;i<20;i++){sensor[i].tempF=555;sensor[i].tempC=555;}}
        ...
        */
         ...
         if (usbNano <0) {perror(MODEMDEVICE); exit(-1); }
...
//		printf("indx=%d pEnd->%s\n",indx,pEnd);
		t=strtof(pEnd,&pEnd);
		if (t<MAX_C_TEMP && t>MIN_C_TEMP) sensor[indx].tempC=t;
//		printf("tempC=%f\n",strtof(pEnd,&pEnd));
		pEnd+=10;
...
//		printf("%s\n",line);
	...
	if (strncmp(line,"No more addresses.",18)==0)return 0;
	return 1; 	
/*	FIELD=0;
