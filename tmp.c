...
"Const SOC 80%   ", "70% 02:00-16:00 ", "Volt Test 50.0  ", "Volt Test 48.8  ",
"Volt Test 48.0  ", "Volt fac2 49.0  ", "Mild Dmnd&Sun Md", "Mild Demand&Sun ",
"11 Modified 3   "
};
...
const float ArrayRatedWatts = 245.0 * 30.0,
	BatRatedAmpHr = 225.0 /*rating */  * 1.5 /*# of strings */ ;
...
float SellWH,
  ...
  netbattamps = 0.0,
	BatTrgAmp = 0.0,
	MaxNegBatAmpsDropped = (-MAX_NEG_NBA_DROPPED);
float DaysSinceFull = 999.9;
float WHtopMaxTemp = 180.1,
	WHtopMinTemp = 100.0,
...
int InverterPwr = 0,
	...
	SellVoltMax = DEFAULT_SELL_V_MAX,
	UnderUtilization;
...
logMesg (const char *fmt, ...)
{
	...
	va_start (ap, fmt);
	wprintw (ScrollWin, "%.2d:%.2d:%.2d:%.2d ", dow, hour, minute,
		 second);
...
//              exit(1);
	  ...
	  {
		  fprintf (mmlog, "%.1d:%.2d:%.2d:%.2d ", dow, hour, minute,
			   second);
...
//              wprintw(ScrollWin,"Error writing to usb\n");
	...
	write (usbmate, ">", 1);
	WMVPRINTW (FNDCWin, 9, 2, "LastCMD <%7s:%-4s>%4d %4d", cmd, v,
		   ibookmark, iLastBookmark);
	...
	if (((Camps > 10.0) || (Camps < (-1.0))) && (Cstart > 0))
	  {
		  CwattHours =
			  (Camps * FNDC_BATT_VOLTS *
			   (float) ((difftime (epoch_time, Cstart)) /
				    3600.0));
	...
	if (sqlite3_prepare_v2 (MMDb, sql, strlen (sql) + 1, &stmt, NULL) ==
	    SQLITE_OK)
	  {
		  if (sqlite3_step (stmt) == SQLITE_ROW)
		    {
			    IAO_Day_Secs =
				    (long) ((sqlite3_column_double (stmt, 0) *
					     3600.0) / 1.08);
			    BuyWH = (float) sqlite3_column_double (stmt,
								   1) *
				    1000.0;
			    InvWH = (float) sqlite3_column_double (stmt,
								   2) *
				    1000.0;
			    DroppedSecs = sqlite3_column_int (stmt, 3) * 60;
			    PowerOutageSecs =
				    sqlite3_column_int (stmt, 4) * 60;
			    UnderUtilizationSecs =
				    sqlite3_column_int (stmt, 5) * 60;
			    BatAmpHrIn =
			    ...
			    BattEffFac =
				    (float) sqlite3_column_double (stmt, 9);
	...
	if ((Prog != PrgDataStruct.Indx) || (HOD != PrgDataStruct.HOD))
	  {
		  ...
		  if (sqlite3_prepare_v2
		      (ppDb, sql, strlen (sql) + 1, &stmt, NULL) == SQLITE_OK)
		    {
			    if (sqlite3_step (stmt) == SQLITE_ROW)
			      {
				      PrgDataStruct.Indx =
		  ...
		  if (sqlite3_prepare_v2
		      (ppDb, sql, strlen (sql) + 1, &stmt, NULL) == SQLITE_OK)
		    {
			    if (sqlite3_step (stmt) == SQLITE_ROW)
			      {
				      PrgDataStruct.NextIndx =
					      sqlite3_column_int (stmt, 0);
				      PrgDataStruct.NextHOD =
					      sqlite3_column_int (stmt, 1);
				      PrgDataStruct.NextSOC_Targ =
					      sqlite3_column_int (stmt, 2);
				      PrgDataStruct.NextSellV =
					      sqlite3_column_int (stmt, 3);
				      PrgDataStruct.Nextfactor =
					      sqlite3_column_int (stmt, 4);
				      strncpy (PrgDataStruct.Description,
					       (char *)
					       sqlite3_column_text (stmt, 5),
...
SaveStats (float Volts, int SOC)
{
	...
	sprintf (sql,
		 ...
		 epoch_time, Volts /*FNDC_BATT_VOLTS */ , SOC /*FNDC_SOC */ ,
		 WaterHeaterKWH, BuyWH / 1000.0, InvWH / 1000.0
		 , CC1_KWHH, CC2_KWHH, (DroppedSecs / 60),
		 (PowerOutageSecs / 60), (UnderUtilizationSecs / 60),
		 BatAmpHrIn, BatAmpHrOut, DischargeAllowed, hour,
		 /*MaxVNegAmps */ BattEffFac, BatAmpHrCrx);
	if (sqlite3_prepare_v2 (MMDb, sql, strlen (sql) + 1, &stmt, NULL) ==
	    SQLITE_OK)
	  {
...
WH_KWH_LastSec (void)
{
	...
	power = (power /
		 (1000.0 /*W to KW */  * 60.0 /*min */  * 60.0 /*sec */ )) *
...
/*	if((dow==1) || (dow==3) || (dow==5))*/
		    ...
		    case 14:
			    ...
			    if (TimeTest (14, 15, 0))
			      {
				      SEND_SMS_STAT_RPT if (BathDone == FALSE)
					{
						if (WHtopMinTemp < 140.0)
							WHtopMinTemp = 160.0;
						if (WHCenterMinTemp < 125.0)
							WHCenterMinTemp =
								140.0;
					}
			      }
		  ...
		  if (TimeTest (28, 58, 30))
		    {
			    sprintf (strtmp,
				     ...
				     CC1_KWHH + CC2_KWHH, (DroppedSecs / 60),
				     (PowerOutageSecs / 60),
				     (UnderUtilizationSecs / 60), BatAmpHrIn,
				     BatAmpHrOut, SOCstart, FNDC_SOC);
//              sendmail("2814509680@vtext.com","garlee55@gmail.com","Report",strtmp);
			    sendmail ("garlee55@gmail.com",
				      "garlee55@gmail.com", "Report", strtmp);
//              BatAmpHrOut=0.0;
		    }
	  }
	...
	if (INVERTER_OP_MODE != INV_IS_OFF)	// inverter is on
	  {
		  if ((FNDC_BATT_VOLTS < 44.0)
		      || (FNDC_SOC <
			  MIN (MAX ((PrgDataStruct.SOC_Targ), (50)), (80))
			  && (InvIgnLowVolt == false)))
		    {
	  ...
	  {
		  if ((FNDC_BATT_VOLTS > 50.0 && FNDC_SOC > 85)
		      || InvIgnLowVolt == true)
		    {
...
{				//need to fix offset mode -- drop grid if input exceedes output by the target
	static int pause = 0;
	enum CC_Modes CC_Mode1 = CC1_MODE,
		CC_Mode2 = CC2_MODE;
...
/*		wprintw(ScrollWin,"Charger trig h%dm%d@%d %lu\t%lu\t%lu\t%d\t%d\t%d\t%lu\n",hour,minute,__LINE__
	...
	if (AdjustedSellV > SUPPORT_MODE_MAX_ADJ_SELL_VOLTS)
		AdjustedSellV =
			MAX (SUPPORT_MODE_MAX_ADJ_SELL_VOLTS,
			     ((float) PrgDataStruct.SellV));
		  ...
		  if (AC1InLimit < (BuyCurMax + 2) && (pause < 1))
		    {
			    ...
			    WMVPRINTW (FNDCWin, 5, 2, "AC1InLimit %4d   ",
				       AC1InLimit);
			    pause = 20;	//10/27/2015
			    return;	//10/27/2015            
		    }
...
/*		if (INVERTER_OP_MODE==IOM_OFFSET){
		  ...
		  if ((((FNDC_BATT_VOLTS * 10) == sellv)
		       || (INVERTERVOLTS == sellv)) && (AC1InLimit >= 5))
		    {
			    pause = 30;
			    return;
		    }
		  ...
		  if (abs (LoadDifWats) > 300)
		    {
			    ...
			    WMVPRINTW (FNDCWin, 5, 2, "NewLimit %4d    ",
				       NewLimit);
			    if (NewLimit < 5)
			      {
				      if ((NewLimit < (-5))
					  && (FNDC_BATT_VOLTS > 51.2))
					{
...
//                                      whsSetFlags(whsAuto,whsOff);    
				      ...
				      if ((INVERTERVOLTS < sellv)
					  || (ANBA > BatTrgAmp)
					  || (NewLimit > AC1InLimit))
					{
						if ((NewLimit >= AC1InLimit)
						    || (BuyCurMax >=
							NewLimit))
						  {
							  sprintf (strtmp,
...
//int   sellAMPs=L1_SELL_AMPS+L2_SELL_AMPS-L1_BUY_AMPS-L1_BUY_AMPS;
	...
	weight +=
		((ANGP >
		  0) ? (ANGP - TargetGPUse) : ((ANGP * 2) - TargetGPUse) * 2);
...
//                                      whsSetFlags(whsAuto,whsOff);
		    ...
		    {
			    if ((TargetGPUse < 1000) && (loadBalance != -1)
				&& GT_NoDrop)
			      {
				      loadBalance = (-1);
	...
	if ((DropSelected) && ((FNDC_SOC) > (MIN_SOC_DROPPED)))
	  {
		  ...
		  if (((MaxNegBatAmpsDropped) * INVEFF) <
		      (ANBA - ((ANGP * 10) / AFNDCV)))
		    {
			    if (tmr++ > 30)
			      {
	...
	if (MrCoolMode == 1)	//1 is cool
	  {
		  if ((MCRoomTemp <= MC_Off_Temp)
		    ...
		    {
		    setACPS (acpsNone)};
		  if ((MCRoomTemp >= MC_On_Temp)
		      && (AirCondPwrSrc == acpsNone))
		    {
		    setACPS (acpsOn)};
	  }
	else			//0 is heat
...
//need to make this play well with other logic  if (vacation!=TRUE) mrCool();    !!!!!!!!!  10.14.2016
	...
	UnderUtilization = ((CC1Mode == 1) || (CC1Mode == 3) || (CC2Mode == 1)
			    || (CC2Mode == 3));
	ngp = ((L1_CHARGER_AMPS + L1_BUY_AMPS -
		L1_SELL_AMPS) * L1_INVERTER_VOUT) + ((L2_CHARGER_AMPS +
						      L2_BUY_AMPS -
						      L2_SELL_AMPS) *
						     L2_INVERTER_VOUT);
	updateANGP ();
	...
	netbattamps =
		((FNDC_SHUNT_A_AMPS) + (FNDC_SHUNT_B_AMPS) +
		 (FNDC_SHUNT_C_AMPS));
	updateANBA ();
	if (netbattamps > 0.0)
	  {
		  if ((FNDC_BATT_VOLTS <= 51.2)
		      || ( /*netbattamps */ ADJNETBATTAMPS >
			  ((FNDC_BATT_VOLTS - 51.0) * 1.6))
		      || (BATT_STATUS_AH < (-0.05)))
		    {
			    BatAmpHrIn += (netbattamps / (60.0 * 60.0));
		    }
		  if (ADJNETBATTAMPS /*netbattamps */  > 1.0)
			  ContinousNegAmpsSec = 0;
	...
	else
	  {
		  ...
		  if ((ContinousNegAmpsSec > 0) || (ADJNETBATTAMPS < (-1.0)))
		    {
			    ContinousNegAmpsSec++;
			    if ((FNDC_BATT_VOLTS > MaxVNegAmps)
				&& (ContinousNegAmpsSec > 60))
		  ...
		  if (digitalRead (WH_LOWER_ELEMENT_HV))
		    {
				...
				&&
				((L2_INVERTER_AMPS + L2_BUY_AMPS -
				  L2_SELL_AMPS) >= WH_Amps_240v))
			      {
				      IAR_Day_Secs++;
			      }
			    else if ((L2_INVERTER_AMPS + L2_BUY_AMPS -
				      L2_SELL_AMPS) >= WH_Amps_120v)
			      {
				      IAO_Day_Secs++;
			      }
		    }
		    ...
		    {
			    if ((L2_INVERTER_AMPS + L2_BUY_AMPS -
				 L2_SELL_AMPS) >= WH_Amps_120v)
				    IAO_Day_Secs++;
		    }
	  }
...
//                              ResetAccumulators();
		    ...
		    case ';':
			    ...
			    break;
		    case ':':
			    if (digitalRead (WH_LOWER_ELEMENT_SRC) ==
				INVERTER)
			      {
				      WhLEGridGoal =
					      readSensorF (4, 666.6) + 5.0;
...
printStuff (void)
{
	...
	WMVPRINTW (InvWin, 4, 2, "Chg   %2d %2d Amps", L1_CHARGER_AMPS,
		   L2_CHARGER_AMPS);
	WMVPRINTW (InvWin, 5, 2, "Buy   %2d %2d %6.3f ", L1_BUY_AMPS,
		   L2_BUY_AMPS, BuyWH / 1000);
	...
	WMVPRINTW (InvWin, 8, 2, "Aux  %2d%2d%2d %2d %2d %2d",
		   ...
		   (digitalRead (WH_LOWER_ELEMENT) != 0),
		   ( /*INVERTER_AUX_OUT!=0 */ compressor (ASK)), (vacation),
		   digitalRead (AIR_JORDAN_SRC_PIN));
	WMVPRINTW (InvWin, 9, 1, "InvPwr %5d %6.3f ", InverterPwr,
		   InvWH / 1000);
	...
	WMVPRINTW (InvWin, 12, 1, "room temp %4.1f      ",
		   /*sensor[3].tempF */ readSensorF (3, -666.9));
	WMVPRINTW (InvWin, 12, 1, "Wall %5.1f atic %5.1f %1s ",
		   sensor[6].tempF, readSensorF (5, -666.9),
		   ((MrCoolMode == 1) ? "C" : "H"));
	WMVPRINTW (InvWin, 13, 1, "On %4.1f Off %4.1f %1s    ",
		   /*sensor[5].tempF */ MC_On_Temp, MC_Off_Temp,
		   (MC_On_Temp > MC_Off_Temp) ? "C" : "H");
	WMVPRINTW (CCWin, 0, 2, "BattV  %4.1f %4.1f", CC1_BATT_VOLTS,
		   CC2_BATT_VOLTS);
	WMVPRINTW (CCWin, 1, 2, "%s", "                      ");
	WMVPRINTW (CCWin, 1, 2, "Prg %1d %s", ProgramIndx,
		   PrgDataStruct.Description /*Programs[ProgramIndx] */ );
	WMVPRINTW (CCWin, 2, 2, "KWH    %4.1f %4.1f %4.1f", CC1_KWHH,
		   CC2_KWHH, CC1_KWHH + CC2_KWHH);
	...
	WMVPRINTW (CCWin, 9, 2, "Ctrl Rm %5.1f %5.1f ",
		   readSensorF (3, -666.9), WhLEGridGoal);
	WMVPRINTW (CCWin, 10, 2, "%02d:%02d:%02d  %02d:%02d:%02d", hour,
		   minute, second, ResetTime_p.tm_hour, ResetTime_p.tm_min,
	...
	WMVPRINTW (CCWin, 12, 2, "%5.1f %5.1f %3d%% >%3.0f",
		   readSensorF (4, 666.6), readSensorF (1, 666.6),
		   LE_PWM.percent, WHCenterMinTemp);
	WMVPRINTW (CCWin, 13, 2, "%1s  %5.1f     %5.1f",
		   ((digitalRead (WH_LOWER_ELEMENT_SRC) ==
		     INVERTER) ? "I" : "G"), readSensorF (0, 666.6),
		   readSensorF (7, 777.7));

	WMVPRINTW (FNDCWin, 0, 2, "BattV %3.1f ",
		   AFNDCV /*FNDC_BATT_VOLTS */ );
	WMVPRINTW (FNDCWin, 1, 2, "SOC   %4d%% Days %4.1f    G %06.0f ",
		   FNDC_SOC, DaysSinceFull, CwattHoursGen);
	...
	WMVPRINTW (FNDCWin, 4, 2, "InvOut/In %6.1f%% ",
		   (INVPWR / ((0 - FNDC_SHUNT_A_AMPS) * FNDC_BATT_VOLTS)) *
		   100);
	if (InvInputMode != GridTied)
	...
	WMVPRINTW (FNDCWin, 7, 1, "MaxNegA Dropped %5.1f ",
		   MaxNegBatAmpsDropped);
...
main (int argc, char *argv[])
{
	...
	if (sqlite3_open_v2
	  ...
	  {
		  WMVPRINTW (FNDCWin, 5, 2, "%14s %d %d %d", "sqlite3 failed",
			     hour, minute, __LINE__);
		  return 3;
	  }
	if (sqlite3_open_v2
	  ...
	  {
		  WMVPRINTW (FNDCWin, 6, 2, "%14s %d %d %d", "sqlite3 failed",
			     hour, minute, __LINE__);
		  return 4;
	  }
	USB_init ();
