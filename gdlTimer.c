...
{
	printf("%lu\t%lu\t%lu\t%d\t%d\t%d\n",t->ReAlarmWaitTimer,t->ReAlarmWaitDelay,t->TriggerCountWithinTimerStart
...
int	TriggerTimer(struct timer_data * t) //returns 0 for no alarm.  1 means alarm.
{
	...
	if (Ticks==t->CurrentTick) return 0; //if ticks hasn't incremented then ignore this call
	...
	else if ((t->TriggerCountWithinTimerStart + t->reqTriggersWithinTicks)<(Ticks+remainingTriggers)) 
